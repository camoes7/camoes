...
$campos=$ligacao->query($sql);
if ($campos->num_rows==0){
	echo '<script>alert("Não há campos disponíveis para realização de marcações.");</script>';
	redirect("index.php");
	die();
}
?>
<div  class="container">
	...
	if (!isset($_POST['nome_do_campo'])){
		...
		<div class="form-group row">
			...
			<div class="col-md-12">
				<label>Selecione o campo pretendido<span style="color:red;"></span></label>
				<select name="Campo">
					<!--option disabled selected>Selecione o campos pretendido</option-->
					<?php
					$numregistos=$campos->num_rows;
					
					for($i=0;$i<$numregistos;$i++){
						
						$registo=$campos->fetch_array(MYSQLI_ASSOC);
						
						$id=$registo['id'];
						$nome=$registo['nome_do_campo'];
	...
	<?php
		...
		for ($i=0;$i<7;$i++){
			...
			if (date('w', strtotime($dia))>0){
				...
				for ($j=0;$j<11;$j++){
					...
					if ($j<> 5){
						$sqlmarcacao="select * from marcacoes where data='$dia' and horamarcacao='$hora' and nome_do_campo=$campos";
						...
						if ($marcacao->num_rows>0){
							...
							if ($regmarcacao['idcliente']==$_SESSION['id_user']){
								echo '<td><a href="includes/pages/anulamarcacao.php?prof='.$campos.'&dia='.$dia.'&hora='.$hora.'"><img src="imagens/desmarcar.png"></a></td>';
							}
							else {
							...
							echo '<td><a href="includes/pages/confirmamarcacao.php?prof='.$campos.'&dia='.$dia.'&hora='.$hora.'"><img src="imagens/livre.png"></a></td>';
						}
					}
				}
			}
		...
		if (!isset($_POST['inicio'])){
			...
			$data=$inicio;
			for($i=0;$i<7;$i++){
				$sql="select * from marcacoes where data='$inicio' and nome_do_campo=$campos";	
			}
		}
	}
	?>
